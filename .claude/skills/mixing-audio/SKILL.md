...
**An explicit `window_s` wider than `clip_duration - hop_s` raises `WindowTooWideForClip`** (0.0.53, issue #43). Above that the clip holds a single window, so the vote you asked for cannot be held: before, you got one whole-clip correlation and a `support=None` that looked exactly like a clip too short to support at any window. The error is importable from `mixing` or `mixing.errors`, subclasses `ValueError`, and carries `clip_index`, `clip_duration_s`, `window_s`, `hop_s` and `max_window_s` — the largest window that still leaves a second look, so a caller putting a mixed-length set on one scale can retry at a window every clip can hold. `max_window_s <= 0` means the clip is shorter than the hop and no window works. This is the explicit path only: `window_s=None` still fits the window to each clip and answers every clip, including one that lands on a single window at the floor.
